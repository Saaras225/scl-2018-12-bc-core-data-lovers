...
//
const resultDataBlurb = dataBlurb(data);
const resultDataStats = dataStats(data);
const resultDataStatHp = dataStatHp(resultDataStats);
const resultStatMp = dataStatMp(resultDataStats);
const resultDataStatMoveSpeed = dataStatMoveSpeed (resultDataStats);
const resultDataStatArmor = dataStatArmor (resultDataStats);
const resultDataStatAttackRange = dataStatAttackRange (resultDataStats);
const resultDataStatHpRegen = dataStatHpRegen (resultDataStats);
const resultDataStatMpRegen = dataStatMpRegen (resultDataStats);
const resultDataStatTags = dataStatTags (data);*/
...
document.getElementById('btn-fighter').addEventListener('click', (event) => {
    ...
    const condition = 'Fighter';
    document.getElementById('root').innerHTML = " ";
    const resultFilterData = filterData(window.data,condition);
    const resultDataImg = dataImg(resultFilterData);
    const resultDataName = dataName(resultFilterData);
    for (let i = 0; i < resultDataImg.length; i++) {
        document.getElementById('root').innerHTML += '<button><img src="' + resultDataImg[i] + '"><p>' + resultDataName[i] + '</p></button>';
...
document.getElementById('btn-search').addEventListener('click', (event) => {
    ...
    const condition = document.getElementById('search').value;
    document.getElementById('root').innerHTML = " ";
    const resultFilterData = dataSearch(data,condition);
    const resultDataImg = dataImg(resultFilterData);
    const resultDataName = dataName(resultFilterData);
    for (let i = 0; i < resultDataImg.length; i++) {
        document.getElementById('root').innerHTML += '<button><img src="' + resultDataImg[i] + '"><p>' + resultDataName[i] + '</p></button>';
